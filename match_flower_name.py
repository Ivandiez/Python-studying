dict_of_flowers = {}
usr_input = input(f"Enter your First [space] Last name only: ")


def create_dict_from_flowers(file):
    with open(file) as f:
        for line in f:
            (key, val) = line.split(": ")
            dict_of_flowers[key] = val[:-1]
    return dict_of_flowers


def user_input_text(some):
    create_dict_from_flowers("flowers.txt")
    for key, value in dict_of_flowers.items():
        if key == usr_input[0]:
            print(f"Unique flower name with the first letter: {dict_of_flowers[key]}")


user_input_text(usr_input)
